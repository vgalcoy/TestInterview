...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 15, 2022 at 08:04 AM
...
--
-- Stand-in structure for view `pitjaruss`
-- (See below for the actual view)
--
CREATE TABLE `pitjaruss` (
...
--
-- Stand-in structure for view `pitjarusss`
-- (See below for the actual view)
--
CREATE TABLE `pitjarusss` (
`area_name` varchar(50)
,`area_id` int(11)
,`testi` decimal(32,0)
,`percentage` varchar(40)
...
--
-- Structure for view `pitjaruss`
--
DROP TABLE IF EXISTS `pitjaruss`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pitjaruss`  AS  select `store_area`.`area_name` AS `area_name`,`store_area`.`area_id` AS `area_id`,count(`report_product`.`compliance`) AS `testi`,concat(round(sum(`report_product`.`compliance`) / 60 * 100,2)) AS `percentage` from ((`report_product` join `store` on(`store`.`store_id` = `report_product`.`store_id`)) join `store_area` on(`store_area`.`area_id` = `store`.`area_id`)) group by `store_area`.`area_id` ;

-- --------------------------------------------------------

--
-- Structure for view `pitjarusss`
--
DROP TABLE IF EXISTS `pitjarusss`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pitjarusss`  AS  select `store_area`.`area_name` AS `area_name`,`store_area`.`area_id` AS `area_id`,sum(`report_product`.`compliance`) AS `testi`,concat(round(sum(`report_product`.`compliance`) / count(`report_product`.`compliance`) * 100,2)) AS `percentage` from ((`report_product` join `store` on(`store`.`store_id` = `report_product`.`store_id`)) join `store_area` on(`store_area`.`area_id` = `store`.`area_id`)) group by `store_area`.`area_id` ;
...
ALTER TABLE `report_product`
  ...
  ADD KEY `fk_relationstore` (`store_id`),
  ADD KEY `fk_relationproduk` (`product_id`);
...
ALTER TABLE `store`
  ...
  ADD KEY `fk_storeaccount` (`account_id`),
  ADD KEY `fk_storearea` (`area_id`);
...
--
ALTER TABLE `report_product`
  MODIFY `report_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=302;
...
--
ALTER TABLE `product`
  ADD CONSTRAINT `fk_productbrand` FOREIGN KEY (`brand_id`) REFERENCES `product_brand` (`brand_id`);

--
-- Constraints for table `report_product`
--
ALTER TABLE `report_product`
  ADD CONSTRAINT `fk_relationproduk` FOREIGN KEY (`product_id`) REFERENCES `product` (`product_id`),
  ADD CONSTRAINT `fk_relationstore` FOREIGN KEY (`store_id`) REFERENCES `store` (`store_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `store`
--
ALTER TABLE `store`
  ADD CONSTRAINT `fk_storeaccount` FOREIGN KEY (`account_id`) REFERENCES `store_account` (`account_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_storearea` FOREIGN KEY (`area_id`) REFERENCES `store_area` (`area_id`);
COMMIT;
